...
| 日期 | 项目 | 提交 | 状态 |
|------|------|------|------|
| 2026-08-06 | 短信读取能力 | 3807ed2 | ✅ 已实施（get_sms_messages，15 测试） |
| 2026-08-06 | 剪贴板能力 | 9b15a35 | ✅ 已实施（get/set_clipboard，15 测试） |
| 2026-08-06 | 通知读取能力 | ee44d33 | ✅ 已实施（get_notifications，12 测试） |
| 2026-08-06 | 通话检测能力 | f2ead58 | ✅ 已实施（get_call_state，12 测试） |
| 2026-08-06 | ADB 命令封装补全 | 6fc23cc | ✅ 已实施（input/app/pm/dumpsys/wm 共 10 方法，67 测试） |
| 2026-08-07 | ROM 兼容性修复（短信/剪贴板/焦点/通知） | 3ea3fe9 | ✅ 已实施（4 项修复 + 真机复验） |
| 2026-08-07 | LLM 闭环接入（4 个查询 ActionType） | dd4aaf2 | ✅ 已实施（查询回传 + prompt 注入，613 passed） |
| 2026-08-07 | 真机端到端回归 | - | ✅ 5/5 全通过（100%），system_prompt 改动无回归 |
| - | MCP 化改造 | - | 暂不涉及 |

> 实施说明：全部在分支 `20260806` 上完成，逐个「更改→测试→提交→推送」。系统级能力统一落在 `ADBController`（复用 `shell()` + 注入防御，失败容错返回安全默认值），测试基线从 447 提升至 568。中文输入仍以 u2 通道优先，ADB 为兜底。
