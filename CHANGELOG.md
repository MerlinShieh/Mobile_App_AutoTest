...
### 验证
- 单元测试：**378 → 398 passed**（0 回归；新增 18 个 pipeline 直测用例，全量通过）
- 覆盖率：**66.2% → 66.88%**（无覆盖率下降；新增 pipeline 直测后升至 **68.71%**）
- 无新增类型错误
- 交叉对比验证（vs 主干 main）：
  - 16 个被迁移方法逐一比对：15 个逐行一致，1 个（`_resolve_scrollable_container`）仅新增防御性判空守卫，行为等价
  - mypy：主干 17 错误 → 重构后 14 错误（消除 step_runner 内 3 个，无新增）
  - 全量测试：主干 380 通过 vs 重构后 380 通过（同环境 .env 对比）
  - 覆盖率：主干 66.67% vs 重构后 66.88%（重构后略高）
  - 新增 `tests/test_pipelines.py`（18 用例）直测三个 pipeline 锁定行为 → 覆盖率升至 68.71%
...
### 新增
- **UI 树摘要输入框标记**（`ui_tree.py`）：`EditText` / 密码输入框节点在结构化摘要中标记为 `输入框`，使 LLM 能直接从文本摘要识别输入框
- **SYSTEM_PROMPT 输入流程强化**：明确"点击输入框只是第一步，绝不代表任务完成"的多步骤流程；新增软键盘指引（点击输入框后软键盘弹出占据下半屏是正常现象，不要误判为页面出错）；新增预填内容处理指引（先 `clear_text` 清空再 `type`，避免文本拼接）
- **`_NO_REASONING_SYSTEM_PROMPT` 同步强化**：保持关闭思维链时两套 Prompt 的输入指引一致

### 验证
- 单元测试：**371 → 378 passed**（新增 7 个用例：ui_tree 输入框标记 3 + decision_prompt 输入指引 4，0 回归）
- mypy：无新增类型错误（lxml stubs 为遗留噪音）

### 待办
- ⏳ **端到端验证**：设备未连接（`adb devices` 无设备），Prompt 强化效果需真实设备跑至少 3 次端到端测试验证
...
| 3 | **LLM Prompt 语义理解**：偶发"只点击搜索框不输入文本"，Prompt 已强化，需端到端反复验证 | bug_analysis.md P0 #1 | P1 | ✅ v1.3.2（输入流程强化 + 输入框标记），⏳ 待端到端验证 |
